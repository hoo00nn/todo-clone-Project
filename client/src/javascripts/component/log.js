...
class Log {
  ...
  makeLogElement = async () => {
    ...
    log.logList.forEach(v => {
      ...
      if(v.state === 'deleted') html += `<span class="log__to">from <strong>${v.log_from}</strong> </span>`;
      else if(v.state === 'added') html += `<span class="log__to">to <strong>${v.log_to}</strong> </span>`;
      else if (v.state === 'moved' && v.log_from === v.log_to) html += `<span class="log__to">the column <strong>${v.log_from}</strong> </span>`;
